...
// TODO Use a foreground or bound service, or a broadcast receiver to prioritize the app.
...
class TrackingActivity : AppCompatActivity() {
    ...
    private fun checkGMSUsable() {
        ...
        if (resultCode == ConnectionResult.SUCCESS) {
            ...
            googleApiAvailability.getErrorDialog(this, resultCode, REQUEST_MAKE_GMS_USABLE) { _ ->
                requestGMS()
            }.show()
        }
    }
...
//            addAction(LocationService.ACTION_LOCATION_REQUEST)
//            addAction(LocationService.ACTION_LOCATION)
//        })
        ...
        if (!viewModel.getLocationUpdate().hasObservers()) {
            viewModel.getLocationUpdate().observeForever { location ->
                Log.e("getLocationUpdate", "$location")
                if (tracking) {
    ...
    private fun checkLocationSettings() {
        ...
        locationSettingsTask.addOnFailureListener { exception ->
            if (exception is ResolvableApiException) {
                try {
                    exception.startResolutionForResult(this@TrackingActivity, REQUEST_CHECK_LOCATION_SETTINGS)
        ...
        locationSettingsTask.addOnSuccessListener { _ ->
            try {
                fusedLocationProvider.requestLocationUpdates(locationRequest, viewModel.getLocationCallback(), null).addOnCompleteListener { locationUpdatesTask ->
...
//                val distance = timeDifference * location.speed
                ...
                if (lifecycle.currentState.isAtLeast(Lifecycle.State.STARTED) && totalDistance > 0.2) {
    ...
    override fun onSaveInstanceState(outState: Bundle?) {
        ...
        Log.e("TrackingActivity", "onSaveInstanceState")
        if (outState != null) {
    ...
    private fun stopUpdates(finishing: Boolean) {
        if (tracking) {
            fusedLocationProvider.removeLocationUpdates(viewModel.getLocationCallback())
            if (!finishing) {
                tracking = false
    ...
    private fun requestLocationPermission() {
        if (ActivityCompat.shouldShowRequestPermissionRationale(this, Manifest.permission.ACCESS_FINE_LOCATION)) {
            ...
            alertDialogBuilder.setPositiveButton("Grant") { _, _ ->
                ActivityCompat.requestPermissions(this, arrayOf(Manifest.permission.ACCESS_FINE_LOCATION), PERMISSION_ACCESS_FINE_LOCATION_PRIORITY)
            }
            alertDialogBuilder.setNegativeButton("Cancel") { _, _ ->
                stopUpdates(false)
            }
